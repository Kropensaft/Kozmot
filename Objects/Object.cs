...
using System.Linq; // Needed for OfType potentially
...
namespace OpenGL // Or OpenGL.Objects based on your using statement
{
    ...
    public class Sphere : Object
    {
            ...
            : base(name, position, rotation, scale, color, mass, planetTypeName, isEmissive) // Pass typeName to base
        {
            ...
            {
                Vector3 relativePos = Position - Parent.Position;
        ...
        public override void Update(double deltaTime)
        {
             ...
             // Assuming Renderer.Spheres is accessible and contains all Sphere objects
            ...
            if (Parent != null)
            {
                ...
                Position = Parent.Position + new Vector3(
                 ...
                 // Reset velocity/acceleration if orbit dictates position? Or let gravity influence orbit slightly?
                 // For pure Keplerian orbit, you might reset Velocity here.
                 // For N-body simulation influenced orbit, you let base.Update handle V/A changes.
                 // Current setup: Gravity modifies V/A, Orbit calculation overrides final Position.
            }
        }
        ...
        private void ApplyGravity(Sphere other, double deltaTime)
        {
            ...
            Vector3 forceDir = direction / distance; // Normalize direction

            // Calculate gravitational force magnitude (F = G * m1 * m2 / r^2)
            float forceMagnitude = Constants.GRAVITATIONAL_CONSTANT * (this.Mass * other.Mass) / distanceSq;

            // Calculate acceleration (a = F / m) and accumulate it
            // Note: We apply change over deltaTime here, which is sometimes debated.
            // Often acceleration is calculated fully, then integrated in Update.
            // Let's accumulate acceleration directly (a = F/m) and integrate in base.Update.
             Vector3 accelerationDueToOther = forceDir * (forceMagnitude / this.Mass);
             this.Acceleration += accelerationDueToOther; // Accumulate acceleration
        }
        ...
        {
            // Use the object's actual scale for radius calculation
            // Assuming Scale.X is the primary radius determinant, or average them?
            // Let's use Scale.X, assuming uniform scaling mostly.
             float radius = this.Scale.X; // Base radius from scale
            ...
            for (int i = 0; i <= stacks; ++i)
            {
                ...
                for (int j = 0; j <= sectors; ++j)
                {
                    ...
                    vertices.Add(this.Color.X);
                    vertices.Add(this.Color.Y);
                    vertices.Add(this.Color.Z);
                }
            }

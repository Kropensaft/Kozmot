...
using System; // Needed for Math.Abs
...
namespace OpenGL
{
    internal static class Indicator
    {
        ...
        {
            var selectedRadius = ImGuiElementContainer.selectedPlanetTypeRef;
        ...
        private static void UpdateIndicatorMeshData()
        {
            ...
            if (Math.Abs(newRadius - _currentRadius) < 0.000001f && _vertices != null && _indices != null)
            {
                return; // Mesh data is up-to-date
            }
            ...
            try
            {
                ...
                if (sphereData.Vertices == null || sphereData.Vertices.Length == 0 || sphereData.Indices == null || sphereData.Indices.Length == 0) {
        ...
        public static void Initialize()
        {
            ...
            // Normal attribute (location = 1) - Assuming your indicator shader uses normals
        ...
        public static void Render(Matrix4 view, Matrix4 projection, Vector3 color, float alpha)
        {
            ...
            if (_vao <= 0 || _vbo <= 0 || _ebo <= 0 || _shaderProgram <= 0 || _vertices == null || _indices == null || _indices.Length == 0)
            {
            ...
            GL.BindBuffer(BufferTarget.ElementArrayBuffer, _ebo); // This binding is implicitly part of the VAO state for drawing, but explicit bind needed for BufferData sometimes.
            GL.BufferData(BufferTarget.ElementArrayBuffer, _indices.Length * sizeof(uint), _indices, BufferUsageHint.DynamicDraw);

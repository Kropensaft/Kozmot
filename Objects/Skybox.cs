...
namespace OpenGL // Or your appropriate namespace
{
    internal class Skybox : IDisposable
    {
        ...
        /// </summary>
        /// <param name="facePaths">Array of 6 paths to the cubemap faces in the order:
        ...
        public Skybox(string[] facePaths, string shaderBasePath = "Shaders")
        {
            ...
            // 1. Load Shaders
            _shaderProgram = Shader.CreateShaderProgram(vertexShaderPath, fragmentShaderPath); // Use your existing Shader helper
        ...
        private int LoadCubemap(string[] faces)
        {
            ...
            StbImage.stbi_set_flip_vertically_on_load(0); // Cubemaps often don't need flipping, adjust if textures are upside down
            ...
            for (int i = 0; i < faces.Length; i++)
            {
                ...
                try
                {
                    ...
                    {
                        ImageResult image = ImageResult.FromStream(stream, ColorComponents.RedGreenBlueAlpha); // Request RGBA

                        if (image.Data != null && image.Width > 0 && image.Height > 0)
                        {
                            // Target faces in OpenGL enum order: +X, -X, +Y, -Y, +Z, -Z
                            TextureTarget targetFace = TextureTarget.TextureCubeMapPositiveX + i;
                            ...
                            Console.WriteLine($"Loaded cubemap face: {targetFace} ({path}) Width={image.Width}, Height={image.Height}");
                        }
                         else {
            ...
            GL.TexParameter(TextureTarget.TextureCubeMap, TextureParameterName.TextureWrapR, (int)TextureWrapMode.ClampToEdge); // Wrap R for the 3rd dimension
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            GL.DeleteProgram(_shaderProgram); // Ensure your Shader class doesn't double-delete if it tracks shaders globally
        }

         // Optional: Add finalizer for safety, although explicit Dispose is better
         ~Skybox()
         {
            // Note: Don't call OpenGL functions here directly if called from finalizer thread!
            // Proper disposal should happen via IDisposable pattern.

...
namespace OpenGL.GUI 
{
    internal abstract class ImGuiElementContainer : IDisposable
    {
        ...
        public static System.Numerics.Vector3 position = new(1f, 0f, 0f); // Use System.Numerics for ImGui
        ...
        public static System.Numerics.Vector3 color = Constants.ROCKY_PLANET_COLOR; // Use System.Numerics for ImGui
        public static List<Object> celestialBodies = new();

        // Use System.Numerics.Vector3 for ImGui ColorEdit3
        private static System.Numerics.Vector3 IndicatorColor = Constants.INDICATOR_COLOR;
                ...
                if (ImGui.BeginTabBar("Settings#left"))
                {
                    ...
                    if (ImGui.BeginTabItem("Planet Creator"))
                    {
                        try
                        {
                            ...
                            if (ImGui.InputText("Planet name", ref nameBuffer, 20)) { } // No action needed on change here
                            ...
                            if (defaultPlanetTypeIndex == 3) // Index 3 is Moon
                            {
                                if (parentNames.Length > 0)
                                {
                                    ...
                                    if (ImGui.Combo("Orbits around", ref selectedParentIndex, parentNames, parentNames.Length)) { } // No action needed on change here
                                }
                                else
                                {
                            ...
                            if (ImGui.DragFloat3("Position (Relative)", ref position, 0.01f, Constants.SLIDER_CLAMP_MIN, Constants.SLIDER_CLAMP_MAX)) { } // No action needed on change here

                            ImGui.SetNextItemWidth(Constants.BESPOKE_TEXTEDIT_WIDTH);
                            if (ImGui.InputText("Mass (e.g., 1.0)", ref massBuffer, 15)) { } // No action needed on change here

                            ImGui.ColorEdit3("Color", ref color);

                            // Ensure emissive checkbox reflects current type unless manually changed
                            if (ImGui.Checkbox("Emissive", ref emissive)) { } // Allow manual override
                    ...
                    if (ImGui.BeginTabItem("Camera settings"))
                    {
                        try
                        {
                            ...
                            if (allBodyNames.Length > 0)
                            {
                                ...
                                ImGui.Combo("Camera Pivot Object", ref selectedPivotIndex, allBodyNames, allBodyNames.Length);

                                if (ImGui.Button("Set Camera Pivot"))
                                {
                                    if (selectedPivotIndex >= 0 && selectedPivotIndex < celestialBodies.Count)
                                    {
                                        Console.WriteLine($"Setting camera pivot to: {celestialBodies[selectedPivotIndex].Name}");
                                        Camera._pivot = celestialBodies[selectedPivotIndex].Position; // Assuming Camera._pivot exists and is OpenTK.Vector3
                                    }
                                    else
                                    {
                            ...
                            if (ImGui.SliderFloat("Indicator Transparency", ref Constants.INDICATOR_ALPHA, 0.0f, 0.9f)) { } // Assuming Constants.INDICATOR_ALPHA exists
        ...
        private static Sphere? SaveUIValues()
        {
            ...
            if (defaultPlanetTypeIndex == 3) // Index 3 is Moon
            {
                ...
                {
                    Console.WriteLine("Warning: Moon type selected, but no valid parent chosen or index out of bounds. Creating as non-orbiting object at specified coords.");
            ...
            switch (defaultPlanetTypeIndex)
            {
                ...
                default: // Fallback to default rocky planet size
                    Console.WriteLine($"Warning: Unknown planet type index {defaultPlanetTypeIndex}. Defaulting to Rocky Planet radius.");
                    radius = Constants.ROCKY_PLANET_RADIUS;
                    break;
            }
            ...
            float angularSpeed = (parent != null || orbitRadius > 0.001f) ? DEFAULT_ANGULAR_SPEED : 0f; // Only orbit if parent or not at origin
        ...
        public static void ResetUI()
        {
            ...
            position = new System.Numerics.Vector3(1f, 0f, 0f);
